...
class OrderAction extends Action {
	public function index() {
		...
		if ($_REQUEST['chaxun'] == 1) 
		{
			...
			{
				$what['users.phone_number'] = array('like','%'.$_REQUEST['hotel'].'%');
			}
		}

	$list = M('orders')->join('users ON users.id = orders.user_id')->join('hotels ON hotels.id = orders.hotel_id')->join('rooms ON rooms.id = orders.room_id')->where($what)->field('orders.*,users.nickname,hotels.name,rooms.display_name,rooms.price,users.phone_number,users.score')->select();
	// var_dump($list);
...
//退款订单
	...
	{
		$what = "(r.order_id = o.id and o.user_id = u.id)";
		if ($_REQUEST['chaxun'] == 1) 
		{
			if (!empty($_REQUEST['hotel'])) 
			{
				$what.="and (u.phone_number like '%".$_REQUEST['hotel']."%')";
			}
		}

		$list = M('refunds')->field('r.*,u.nickname,u.phone_number')->table('refunds r,users u,orders o')->where($what)->select();
		// var_dump($list);
		$this->assign('list',$list);
		$this->display();
	}
...
//未入住订单
	public function onorder()
	{
		...
		if ($_REQUEST['chaxun'] == 1) 
		{
			if (!empty($_REQUEST['hotel'])) 
			{
				$what.="and (users.phone_number like '%".$_REQUEST['hotel']."%')";
			}
		}

		$list = M('orders')->join('users ON users.id = orders.user_id')->join('hotels ON hotels.id = orders.hotel_id')->join('rooms ON rooms.id = orders.room_id')->where($what)->field('orders.*,users.nickname,hotels.name,rooms.display_name,rooms.price,users.phone_number')->select();
		// var_dump($list);
		$this->assign('list',$list);
		$this->display();
	}
