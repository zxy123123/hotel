...
class IndexAction extends Action 
{
    ...
    $admin=M('admin');
        $user=M('hotel_admin');
        $username=trim($_POST['username']);//获取post传过来的用户名
        $password=md5(trim($_POST['password']));//把密码加密
        //$password2=md5($password);
        $condition['name']=$username;
        $condition['password']=$password;//传到$condition中
        $con['Name'] = $username;
        $con['Password'] = $password;
        $query=$user->where($condition)->find();//在hotel_admin表中找
        $query_admin=$admin->where($con)->find();//在admin表中找
        $admin_id = $user->where($con)->getField('admin_id');
        $credit = $user->where($con)->getField('credit');
        //var_dump($user->getLastSql());exit;
        if($query_admin){
            $_SESSION["username"] = $username;
            $_SESSION['admin_id'] = $query_admin['AdminID'];
    ...
    //         $list = M('Hotel_admin')->where(array('name'=>$username))->field('password')->find();
	...
	public function editPwd(){
    	...
    	$query=$user->where($condition)->find();
            $list = $Form->where($row)->find();
    	header("Content-type:text/html;charset=utf-8");
    	if($query)
    	{
			$AdminId = $user->where($condition)->getField('AdminID');
			$con1['AdminID'] = $AdminId;
			$con['Name'] = $username;
        ...
        {
            $AdminId = $Form->where($row)->getField('admin_id');
            $con1['admin_id'] = $AdminId;
            $con['name'] = $username;
            $con['password'] = $newpassword;
            $result = $Form->where($con1)->save($con);
            unset($_SESSION["username"]);
            //$this->success('密码修改成功，请用新密码重新登录', '../../admin.php');
            echo "<script>alert('密码修改成功，请用新密码重新登录');top.location = '../../admin.php';</script>";
        }
