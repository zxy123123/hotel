...
function deleteData() {
	...
	if (rows.length > 0) {
		parent.dj.messagerConfirm('请确认', '您要删除当前所选项目？', function(r) {
			if (r) {
				for ( var i = 0; i < rows.length; i++) {
					ids.push(rows[i].reservation_id);
				}
				$.ajax({
...
function batch(t){
	...
	var rows = datagrid.datagrid('getChecked');
	var ids = [];
	var tn = (t=='confirm')?"接单":"修改订单状态为已入住";
	if (rows.length > 0) {
...
function formatOper(val,row,index){
	var str= '';
	if(row.flag==0){
		str+= '<a href="javascript:void(0);" onclick="accept('+row.reservation_id+')">入住确认</a>'; 
	}
	...
	<table border="1" cellpadding="1" cellspacing="1" width="100%" align="center">
		<thead>
			...
			<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
				...
				<td  >
				<?php if($vo["payment_status"] == paid): ?>已支付
				<?php elseif($vo["payment_status"] == not_paid): ?>未支付
				<?php else: ?>支付失败<?php endif; ?>
				</td>
				<td >
				<?php if($vo["payment_method"] == wechat): ?>微信
				<?php elseif($vo["payment_method"] == score): ?>金币
				...
				<table border="1" >
					...
					<tr>
						<td width="80"><?php echo ($vo["name"]); ?></td>
						<td width="80"><?php echo ($vo["display_name"]); ?></td>
						<td width="80"><?php echo ($vo["price"]); ?></td>
						<td width="80">
						<?php if($vo["order_status"] == tbc): ?>待确认
						<?php elseif($vo["order_status"] == success): ?>成功
						<?php elseif($vo["order_status"] == cancle): ?>取消
						<?php elseif($vo["order_status"] == refunding): ?>退款中
						<?php else: ?>完成<?php endif; ?>
						</td>
						<td width="80"><?php echo ($vo["latest_arrive_time"]); ?></td>
						<td width="80"><?php echo ($vo["open_id"]); ?></td>
						<td width="80"><?php echo ($vo["special_requirement"]); ?></td>
						<td width="80"><?php echo ($vo["remark"]); ?></td>
					</tr>
				</table>
		...
		<td>
		<button class="mask" onclick="remark(<?php echo ($vo["id"]); ?>)">备注</button>
		<div class="stages mask_<?php echo ($vo["id"]); ?>">
		<div class="forms">
		<form action="<?php echo U('Order/mask');?>?id=<?php echo ($vo["id"]); ?>" method="post">
		...
		<button class="check" bh="<?php echo ($vo["id"]); ?>">确认入住</button>
		<button>改签</button>
		<button class="del" bh="<?php echo ($vo["id"]); ?>">删除</button>
